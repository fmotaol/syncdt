...
BEGIN
    ...
    ', schema_name, base_table_name, query, refresh_mode, primary_key);
...
BEGIN
    ...
    IF position('.' in derived_table) > 0 THEN
        derived_schema := split_part(derived_table, '.', 1);
        derived_name := split_part(derived_table, '.', 2);
    ELSE
        derived_schema := 'public';
        derived_name := derived_table;
    END IF;
    ...
    WHERE table_schema = derived_schema AND table_name = derived_name;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'Tabela derivada %.% não encontrada. Crie-a primeiro com syncdt.create_derived_table()',
                        derived_schema, derived_name;
    END IF;

    -- Verifica se já existe dependência
    SELECT * INTO metadata_record
    FROM syncdt._dependencies
    WHERE derived_schema = derived_schema 
      AND derived_table = derived_name
      AND source_schema = source_schema
      AND source_table = source_name;

    IF FOUND THEN
        RAISE WARNING 'Dependência já existe para %.% <- %.%, atualizando...',
                      derived_schema, derived_name, source_schema, source_name;
        
        IF metadata_record.trigger_name IS NOT NULL THEN
            EXECUTE format('DROP TRIGGER IF EXISTS %I ON %I.%I',
                          metadata_record.trigger_name, source_schema, source_name);
        END IF;
        
        DELETE FROM syncdt._dependencies 
        WHERE derived_schema = derived_schema 
          AND derived_table = derived_name
          AND source_schema = source_schema
          AND source_table = source_name;
    ...
    INSERT INTO syncdt._dependencies (
		...
		VALUES (derived_schema, derived_name, source_schema, source_name, target_columns, source_exp, trigger_name, NOW());

    RAISE NOTICE 'Source table %.% atribuída a %.% (trigger: %)', 
                 source_schema, source_name, derived_schema, derived_name, trigger_name;
END;
$$;
...
BEGIN
	...
	CREATE INDEX IF NOT EXISTS idx_refresh_queue_pending 
		ON syncdt._refresh_queue(triggered_at, id) 
		WHERE processed_at IS NULL;

	CREATE INDEX IF NOT EXISTS idx_refresh_queue_locks 
		ON syncdt._refresh_queue(locked_until) 
...
select syncdt.create_derived_table(
	table_name => 'tlm.resumo_leituras_tag_hora',
	query => 'select * from tlm.gerar_resumo_leituras_tag_hora',
	refresh_mode => 'auto'
);

--FALTA TRATAR O POVOAMENTO INICIAL. VAI SER TUDO LENTO, TRAVADO MESMO? OU TEM ALGUMA ESTRATÉGIA DE DEFERRED?


select syncdt.assign_source_table(
		derived_table => 'tlm.resumo_leituras_tag_hora',
		source_table => 'tlm.leitura_tag',
		target_columns => 'diahora_ref, tag',
		source_exp => '${source}.diahora, ${source}.tag'
);

--FALTA VER ESTRATÉGIA DE SINCRONIZAÇÃO - ENTRE A CRIAÇÃO DA TABELA E A VINCULAÇÃO DAS ALTERAÇÕES. 
--TALVEZ PRECISE DE ALGUM MOMENTO TER UM "UNLOCK" PARA USO
