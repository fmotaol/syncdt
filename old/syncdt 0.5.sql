...
BEGIN
    ...
    INSERT INTO syncdt._dependencies (
        ...
        target_columns, source_exp, trigger_name, created_at, index_names
    ) VALUES (
        d_schema, d_name, s_schema, s_name, 
        target_columns, source_exp, trigger_name, NOW(), NULL
    );
...
BEGIN
	...
	if notification_mode = 'optimized' then
	    ...
	    INSERT INTO syncdt._refresh_queue (derived_schema, derived_table, triggered_at, sfilter)
...
BEGIN
    ...
    RAISE NOTICE 'Refresh concluído (deletados: %, inseridos: %, filters: %)', 
                 total_deleted, total_inserted, filter_count;
