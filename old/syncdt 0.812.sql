...
BEGIN
    ...
    IF job_count = 0 THEN
        RAISE NOTICE 'Nenhum job disponível para processar';
        RETURN;
    END IF;
...
select
		...
		triggered_at,
		"filter",
		case when processed_at is not null then 'Processed' 
			 when wait_until is null or wait_until > now() then 'Waiting'
		end as status,
		processed_at,
		wait_until,
		derived_schema,
