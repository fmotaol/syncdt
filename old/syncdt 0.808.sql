	...
	if notification_mode = 'optimized' then
		...
		WHERE NOT EXISTS (SELECT * FROM syncdt._refresh_queue q
		    WHERE q.derived_schema = _notify_source_changed.derived_schema AND q.derived_table = _notify_source_changed.derived_table
		      AND (q.wait_until IS NULL OR q.wait_until < _notify_source_changed.wait_until)
...
DECLARE
    ...
    pid int;
    st text;
    processed_filters TEXT[] := '{}';  -- Array para armazenar filtros já processados
    filter_key TEXT;                    -- Chave única para cada filtro
BEGIN
    start_time := NOW();
    pid = pg_backend_pid();
    ...
    FOR rec IN
        ...
        ORDER BY triggered_at, id ASC
    LOOP
        -- Construir chave única para o filtro (considerando também schema e tabela)
        filter_key := rec.derived_schema || '.' || rec.derived_table || '||' || COALESCE(rec.filter, '');
        
        -- Verificar se este filtro já foi processado nesta execução
        IF NOT (filter_key = ANY(processed_filters)) THEN
            -- Adicionar filtro à lista de processados
            processed_filters := array_append(processed_filters, filter_key);
            ...
            raise notice 'Excluído % registro(s) de %.% => %', rows_affected, rec.derived_schema, rec.derived_table, rec.FILTER;

            filter_count := filter_count + 1;
        ELSE
            raise notice 'Filtro "%" já processado na exclusão, ignorando duplicata para %.%', 
                         rec.filter, rec.derived_schema, rec.derived_table;
        END IF;
    END LOOP;
    
    -- Resetar o array de filtros processados para a fase de INSERT
    processed_filters := '{}';
    ...
    FOR rec IN
        ...
        ORDER BY triggered_at, id ASC
    LOOP
        -- Construir chave única para o filtro
        filter_key := rec.derived_schema || '.' || rec.derived_table || '||' || COALESCE(rec.filter, '');
        ...
        IF NOT (filter_key = ANY(processed_filters)) THEN
            ...
            if not FOUND or query is null then
                raise exception 'definition_query não encontrada para %.%', rec.derived_schema, rec.derived_table;
            end if;

            perform syncdt.execute('DELETE FROM %I.%I WHERE %s', -- É necessário reexcluir, pelo risco de duplicidade causada por filtro "cruzado" 
                rec.derived_schema, rec.derived_table, rec.filter);  -- (i.é, filtros diferentes que afetam conjuntos de dados em comum)

            perform syncdt.execute('INSERT INTO %I.%I select * from (%s) where %s',
                rec.derived_schema, rec.derived_table, query, rec.filter);
            GET DIAGNOSTICS rows_affected = ROW_COUNT;
            raise notice 'Reinserido % registro(s) de %.% => %', rows_affected, rec.derived_schema, rec.derived_table, rec.FILTER;

            total_inserted := total_inserted + rows_affected;
        ELSE
            raise notice 'Filtro "%" já processado na inserção, ignorando duplicata para %.%', 
                         rec.filter, rec.derived_schema, rec.derived_table;
        END IF;

        -- Marca como processado (independente de ter executado ou não)
        UPDATE syncdt._refresh_queue 
...
select syncdt.assign_dependency(
		derived_table => 'tlm.resumo_leituras_tag_dia',
		source_table => 'tlm.tag',
		target_columns => 'tag_id',
		source_exp =>  $$= ${source.id}$$,
);
