...
DECLARE
    ...
    v_formatted_value text;
    v_sql text;
    v_is_record_ph boolean;
    v_is_query_ph boolean;
BEGIN
    v_result := sql_exp;
    ...
    -- Usa regexp_matches para encontrar TODOS os placeholders de registro
    FOR v_match IN 
    ...
    LOOP
        ...
        -- Formata o valor usando a função existente
        v_formatted_value := syncdt.format_sql_value(v_column_value);
        v_result := replace(v_result, v_placeholder, v_formatted_value);
    END LOOP;
    
    -- Segundo passo: resolver placeholders de query (recursivamente)
    -- Encontra todos os placeholders de query
    FOR v_match IN 
        SELECT regexp_matches(v_result, query_ph, 'g')
    LOOP
        v_placeholder := v_match[0];
        v_inner_sql := v_match[1]; -- O conteúdo dentro do placeholder
        ...
        BEGIN
            ...
            -- Substitui o placeholder pelo resultado da execução
            v_result := replace(v_result, v_placeholder, v_query_result);
            
        EXCEPTION WHEN OTHERS THEN
            -- Em caso de erro na execução, mantém o placeholder ou levanta erro?
            RAISE EXCEPTION 'Erro ao executar SQL interno "%": %', v_resolved_sql, SQLERRM;
        END;
    END LOOP;
    
    -- Retorna a expressão completamente resolvida
    RETURN v_result;
END;
$function$;
